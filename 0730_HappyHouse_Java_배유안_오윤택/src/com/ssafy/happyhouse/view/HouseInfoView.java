...
import com.ssafy.happyhouse.util.UseJson;
...
public class HouseInfoView{
	...
	private void showHouseInfo(int code) {
		...
		houseInfoL[0].setText(curHouse.getAptName());
		houseInfoL[1].setText(""+curHouse.getDealAmount());
		String rent = curHouse.getRentMoney();
		if(rent == null) {
			houseInfoL[2].setText("없음");
		}else {
			houseInfoL[2].setText(curHouse.getRentMoney());
		}
		houseInfoL[3].setText(""+curHouse.getBuildYear());
		houseInfoL[4].setText(curHouse.getArea()+"");
		houseInfoL[5].setText(String.format("%d년 %d월 %d일"
											,curHouse.getDealYear()
											,curHouse.getDealMonth()
											,curHouse.getDealDay()
											));
		houseInfoL[6].setText(curHouse.getDong());
		houseInfoL[7].setText(curHouse.getJibun());
		try {
			houseInfoL[8].setText(UseJson.enData(curHouse.getDong())+"개");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		houseInfoL[9].setText("");
...
//		});
	...
	public void setMain(){
		...
		leftR.setBorder(BorderFactory.createEmptyBorder(80 , 10 , 10 , 10));
		
		String[] info= {"주택명","거래금액","월세금액","건축연도","전용면적","거래일","법정동","지번", "환경오염시설", ""};
		...
		houseTable.addMouseListener( new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				showHouseInfo(code);
				JOptionPane.showMessageDialog(mainP, "주변상권 : "+UseCsv.toString(curHouse.getDong())); // 주변상권 정보 팝업
			}
		});
		...
		searchBt.addActionListener( (ActionEvent e) -> searchHouses() );
